...
- (void)initView {
    ...
    _writeValueView.frame = CGRectMake(0, kNewTopHeight, kMainScreenWidth, 160);
    [self.view addSubview:_writeValueView];
    
    _writeValueView.yxWriteValueViewGetListBlock = ^(NSString * _Nonnull saveName, NSString * _Nonnull downAmount, NSString * _Nonnull downName, NSString * _Nonnull saveBegainNum, NSString * _Nonnull downAddress) {
        
        [weakSelf assemblyDataSourceValue:saveName downAmount:downAmount downName:downName saveBegainNum:saveBegainNum downAddress:downAddress];
    };
...
#pragma mark - 组装数据
- (void)assemblyDataSourceValue:(NSString *)saveName downAmount:(NSString *)downAmount downName:(NSString *)downName saveBegainNum:(NSString *)saveBegainNum downAddress:(NSString *)downAddress {
    ...
    for (NSInteger i = begainDownLoadNum; i < endDownLoadNum; i ++) {
        ...
        NSString *downLoadNumStr = [[NSString alloc] init];
        ...
        NSString *downLoadAddress = [NSString stringWithFormat:@"%@/%@.jpg", downAddress, downLoadNumStr];
